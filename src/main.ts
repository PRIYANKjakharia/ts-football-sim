...
async function createPlayer(playerNo: number): Promise<Player> {
...
async function createTeam(teamNumber: number): Promise<Team> {
...
async function chooseCommentary(): Promise<Commentary> {
...
async function chooseTeam(match: Match): Promise<Team> {
...
function showScore(match: Match) {
    ...
    console.log(
        `${match.teamA.getTeamName()} : ${match.score.getTeamAGoals()}`
    );

    console.log(
        `${match.teamB.getTeamName()} : ${match.score.getTeamBGoals()}`
    );

    console.log("========================\n");
}

async function passBall(match: Match) {
    ...
    if (result === "1") match.commentary.goodPass(player!);
    else if(result === "2") match.commentary.badPass(player!);
    else console.log("X---- Invalid Choice🚩 ----X");
}

async function tackle(match: Match) {
...
async function longPass(match: Match) {
...
async function shootBall(match: Match) {
    ...
    if (result === "1") {
        ...
        if (team === match.teamA) match.score.addTeamAGoals(player!, team);
        else match.score.addTeamBGoals(player!, team);
...
async function saveBall(match: Match) {
    ...
    const keeper =
        choice === "1" ? match.teamA.getGoalKeeper() : match.teamB.getGoalKeeper();
...
async function run(match: Match) {
...
async function celebrate(match: Match) {
        ...
        switch (choice) {
            ...
            case "9":
                ...
                if (mom != null) {
                    ...
                    console.log(
                        `Man of the Match : ${mom.getName()} (${mom.getGoals()} Goals)`
                    );
                }
